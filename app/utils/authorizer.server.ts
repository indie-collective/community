...
import isAuthenticated from "./isAuthenticated.server";
...
export class Authorizer<User = unknown, Data = unknown> {
  ...
  async authorize(
    ...
    let user = await isAuthenticated(args.request);

    if (!user) {
      if (raise === "response") {
        throw data({ message: "Not authenticated." }, { status: 401 });
      }
      if (raise === "redirect") {
    ...
    for (let rule of [...this.rules, ...rules]) {
      ...
      if (raise === "response") {
        if (!rule.name) throw data({ message: "Forbidden" }, { status: 403 });
        throw data(
